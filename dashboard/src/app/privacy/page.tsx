...
export default function PrivacyPage() {
  return (
    <div style={{ maxWidth: 720, margin: "0 auto", padding: "48px 24px" }}>
      <Link
        href="/login"
        style={{ fontSize: 13, color: "var(--text-muted)", display: "inline-block", marginBottom: 32 }}
      >
        ← ログインに戻る
      </Link>
      ...
      <Section title="4. 第三者への提供">
        ...
        <p style={{ marginTop: 12 }}>
          本サービスは以下の外部サービスを利用しており、一部データが各社のサーバー（米国）に送信されます。
        </p>
        <ul>
          <li>
            <strong>Supabase, Inc.</strong>（認証・データベース）—{" "}
            <a href="https://supabase.com/privacy" target="_blank" rel="noopener noreferrer">
              プライバシーポリシー
            </a>
          </li>
          <li>
            <strong>Groq, Inc.</strong>（評価スコア算出のためのLLM推論）— 質問・回答データを送信します。{" "}
      ...
      <Section title="6. Cookie・ローカルストレージ">
        <p>
          本サービスはセッション管理のためにCookieおよびローカルストレージを使用します。
      ...
      <Section title="8. 問い合わせ先">
        ...
        <p style={{ marginTop: 8 }}>
          <a href="https://github.com/A-1ro/rag-eval/issues" target="_blank" rel="noopener noreferrer">
